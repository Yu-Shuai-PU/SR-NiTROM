...
training_ic_perturbation_name = "sin(x)"
...
fname_time_training = traj_path + "time_training.npy" # for plotting, in case our training & reconstruction timespan is shorter than the entire simulation timespan

np.save(fname_time_training,opt_obj.time)
...
relative_error = np.zeros(opt_obj.n_snapshots)
relative_error_space_time_SRN = np.zeros(n_traj)
...
for k in range(pool.my_n_traj):
    ...
    if traj_idx == 0:
        ...
        plt.title(f"SRN solution, error: {relative_error_space_time_SRN[k]:.4e}, initial condition = uIC + {amp_array[traj_idx - 1]} * {training_ic_perturbation_name}")
    ...
    if traj_idx == 0:
        ...
        plt.title(f"Shift amount, initial condition = uIC + {amp_array[traj_idx - 1]} * {training_ic_perturbation_name}")
    ...
    if traj_idx == 0:
        ...
        plt.title(f"Shift speed, initial condition = uIC + {amp_array[traj_idx - 1]} * {training_ic_perturbation_name}")
