...
# relative_error_SRG = np.zeros(opt_obj.n_snapshots)                                            
# relative_error_fitted_SRG = np.zeros(opt_obj.n_snapshots)
...
#         disturbance_kinetic_energy_FOM[j] = fom.inner_product_3D(traj_fitted_FOM[0 : params.nx * params.ny * params.nz,j].reshape((params.nx, params.ny, params.nz)),
...
#                                                                traj_fitted_SRG[params.nx * params.ny * params.nz : ,j].reshape((params.nx, params.ny, params.nz)))
#         relative_error_fitted_SRG[j] = fom.inner_product_3D(diff_SRG_FOM_fitted[0 : params.nx * params.ny * params.nz].reshape((params.nx, params.ny, params.nz)),
...
#         relative_error_SRG[j] = fom.inner_product_3D(diff_SRG_FOM[0 : params.nx * params.ny * params.nz].reshape((params.nx, params.ny, params.nz)),
...
#     plot_SRG_vs_FOM(opt_obj, traj_idx, params.fig_path_SRG, relative_error_SRG, relative_error_fitted_SRG,
...
#                     traj_fitted_FOM_proj_POD, sol_SRG,
#                     traj_FOM, traj_SRG,
...
#     np.save(f"{params.traj_path_FOM}disturbance_kinetic_energy_traj_{traj_idx:03d}.npy", disturbance_kinetic_energy_FOM)
#     np.save(f"{params.traj_path_SRG}disturbance_kinetic_energy_traj_{traj_idx:03d}.npy", disturbance_kinetic_energy_SRG)
#     np.save(f"{params.traj_path_SRG}relative_error_traj_{traj_idx:03d}.npy", relative_error_SRG)
#     np.save(f"{params.traj_path_SRG}relative_error_fitted_traj_{traj_idx:03d}.npy", relative_error_fitted_SRG)
...
# region 2: Training the SR-NiTROM ROM 
...
# Gr_Phi_w = manifolds.Grassmann(opt_obj.n_states, params.r)
# if params.manifold == "Stiefel":
#     Gr_Psi_w = manifolds.Stiefel(opt_obj.n_states, params.r)
# elif params.manifold == "Grassmann":
#     Gr_Psi_w = manifolds.Grassmann(opt_obj.n_states, params.r)
...
# region 3: Simulate the trained SR-NiTROM ROM
...
disturbance_kinetic_energy_SRN = np.zeros(opt_obj.n_snapshots)
relative_error_SRN = np.zeros(opt_obj.n_snapshots)
relative_error_fitted_SRN = np.zeros(opt_obj.n_snapshots)
traj_SRN = np.zeros((opt_obj.n_states, opt_obj.n_snapshots)) # (my_n_traj, n_states, n_snapshots)
shifting_speed_SRN = np.zeros(opt_obj.n_snapshots) # (my_n_traj, n_snapshots)
...
for k in range(pool.my_n_traj):
    ...
    sol_SRN = solve_ivp(opt_obj.evaluate_rom_rhs,
                    [opt_obj.time[0],opt_obj.time[-1]],
    ...
    traj_fitted_SRN = fom.apply_inv_sqrt_inner_product_weight(PhiF_NiTROM_w@sol_SRN[:-1,:])
    traj_fitted_SRN_v = traj_fitted_SRN[0 : params.nx * params.ny * params.nz, :].reshape((params.nx, params.ny, params.nz, -1))
    traj_fitted_SRN_eta = traj_fitted_SRN[params.nx * params.ny * params.nz : , :].reshape((params.nx, params.ny, params.nz, -1))
    shifting_amount_SRN = sol_SRN[-1,:]
    
    traj_FOM, traj_fitted_FOM, traj_fitted_weighted_FOM = opt_obj.load_various_FOM_trajectories_idx(pool, k)
    disturbance_kinetic_energy_FOM = np.load(f"{params.traj_path_FOM}disturbance_kinetic_energy_traj_{traj_idx:03d}.npy")
    disturbance_kinetic_energy_SRG = np.load(f"{params.traj_path_SRG}disturbance_kinetic_energy_traj_{traj_idx:03d}.npy")
    relative_error_SRG = np.load(f"{params.traj_path_SRG}relative_error_traj_{traj_idx:03d}.npy")
    relative_error_fitted_SRG = np.load(f"{params.traj_path_SRG}relative_error_fitted_traj_{traj_idx:03d}.npy")
    shifting_amount_SRG = np.load(f"{params.traj_path_SRG}shifting_amount_traj_{traj_idx:03d}.npy")
    shifting_speed_SRG = np.load(f"{params.traj_path_SRG}shifting_speed_traj_{traj_idx:03d}.npy")
    traj_SRG = np.load(f"{params.traj_path_SRG}traj_SRG_traj_{traj_idx:03d}.npy")
    traj_fitted_SRG = np.load(f"{params.traj_path_SRG}traj_fitted_SRG_traj_{traj_idx:03d}.npy")
    sol_SRG = np.load(f"{params.traj_path_SRG}sol_SRG_traj_{traj_idx:03d}.npy")
    traj_fitted_FOM_proj_POD = np.load(f"{params.traj_path_SRG}traj_fitted_FOM_proj_POD_traj_{traj_idx:03d}.npy")
    
    for j in range (len(opt_obj.time)):
        traj_SRN_v_vec = fom.shift_x_input_3D(traj_fitted_SRN_v[:, :, :, j], shifting_amount_SRN[j])
        traj_SRN_eta_vec = fom.shift_x_input_3D(traj_fitted_SRN_eta[:, :, :, j], shifting_amount_SRN[j])
        traj_SRN[:,j] = np.concatenate((traj_SRN_v_vec.ravel(), traj_SRN_eta_vec.ravel()))
        shifting_speed_SRN[j] = opt_obj.compute_shift_speed(sol_SRN[:-1,j], Tensors_NiTROM_w)
        diff_SRN_FOM_fitted = traj_fitted_SRN[:,j] - traj_fitted_FOM[:,j]
        diff_SRN_FOM        = traj_SRN[:,j] - traj_FOM[:,j]
        disturbance_kinetic_energy_SRN[j] = fom.inner_product_3D(traj_fitted_SRN[0 : params.nx * params.ny * params.nz,j].reshape((params.nx, params.ny, params.nz)),
                                                               traj_fitted_SRN[params.nx * params.ny * params.nz : ,j].reshape((params.nx, params.ny, params.nz)),
                                                               traj_fitted_SRN[0 : params.nx * params.ny * params.nz,j].reshape((params.nx, params.ny, params.nz)),
                                                               traj_fitted_SRN[params.nx * params.ny * params.nz : ,j].reshape((params.nx, params.ny, params.nz)))
        relative_error_fitted_SRN[j] = fom.inner_product_3D(diff_SRN_FOM_fitted[0 : params.nx * params.ny * params.nz].reshape((params.nx, params.ny, params.nz)),
                                                 ...
                                                 diff_SRN_FOM_fitted[params.nx * params.ny * params.nz : ].reshape((params.nx, params.ny, params.nz))) / disturbance_kinetic_energy_FOM[j]
        relative_error_SRN[j] = fom.inner_product_3D(diff_SRN_FOM[0 : params.nx * params.ny * params.nz].reshape((params.nx, params.ny, params.nz)),
                                                 ...
                                                 diff_SRN_FOM[params.nx * params.ny * params.nz : ].reshape((params.nx, params.ny, params.nz))) / disturbance_kinetic_energy_FOM[j]
        
    traj_fitted_FOM_proj_NiTROM = Psi_NiTROM_w.T @ traj_fitted_weighted_FOM
    ### plotting
    plot_SRN_vs_FOM(opt_obj, traj_idx, params.fig_path_SRN,
                    relative_error_SRG, relative_error_SRN,
                    relative_error_fitted_SRG, relative_error_fitted_SRN,
                    disturbance_kinetic_energy_FOM, disturbance_kinetic_energy_SRG, disturbance_kinetic_energy_SRN,
                    opt_obj.c[k,:], shifting_amount_SRG, shifting_amount_SRN,
                    opt_obj.cdot[k,:], shifting_speed_SRG, shifting_speed_SRN,
                    traj_fitted_FOM_proj_POD, traj_fitted_FOM_proj_NiTROM, sol_SRG, sol_SRN,
                    traj_FOM, traj_SRG, traj_SRN,
                    traj_fitted_FOM, traj_fitted_SRG, traj_fitted_SRN,
                    params.num_modes_to_plot, params.nx, params.ny, params.nz, params.dt, params.nsave,
