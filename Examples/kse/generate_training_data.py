...
n_traj = 9
traj_init_array = np.zeros((nx, n_traj))
traj_init_unperturbed = np.loadtxt(f"initial_condition_time_{start_time}.txt").reshape(-1) 
amp = 1.0
amp_array = np.array([-1.0, -0.5, -0.2, -0.1, 0.1, 0.2, 0.5, 1.0]) * amp
ic_perturbation_name = "sin(x)"

traj_init_array[:, 0] = traj_init_unperturbed
for idx_k, k in enumerate(amp_array):
    traj_init_array[:, idx_k + 1] = traj_init_unperturbed + k * np.sin(2 * np.pi / L * x)
    # print(f"initial condition No.{idx_k + 1}, perturbation amp proportion = {np.sqrt(np.linalg.norm(traj_init_array[:, idx_k + 1] - traj_init_unperturbed)**2 / np.linalg.norm(traj_init_unperturbed)**2):.4f}")
...
for k in range (pool.my_n_traj):
    ...
    traj, tsave = tstep_kse_fom.time_step(X0, nsave)
    traj_fitted, shift_amount = fom.template_fitting(traj, X_template)
    X0_fitted = traj_fitted[:,0]
    ...
    np.save(fname_traj_init%traj_idx,X0)
    np.save(fname_traj_init_fitted%traj_idx,X0_fitted)
    ...
    if traj_idx == 0:
        ...
        plt.title(f"FOM solution, initial condition = uIC + {amp_array[traj_idx - 1]} * {ic_perturbation_name}")
    plt.savefig(fig_path + "traj_%03d.png"%traj_idx)
    plt.close()
    ...
    if traj_idx == 0:
        ...
        plt.title(f"FOM solution (fitted), initial condition = uIC + {amp_array[traj_idx - 1]} * {ic_perturbation_name}")

    plt.savefig(fig_path + "traj_fitted_%03d.png"%traj_idx)
    plt.close()
    ...
    plt.savefig(fig_path + "Fourier_components_%03d.png"%traj_idx)
    plt.close()

    # print the final shift amount
    print(f"Trajectory No.{traj_idx}, final shift amount: %.4f"%(shift_amount[-1]))
    
    # # print the L2 norm of the initial condition
    # print(f"rank = {pool.rank}, L2 norm of the initial condition: {np.linalg.norm(X_init_array[:, traj_idx])}")
